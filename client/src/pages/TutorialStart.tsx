...
export const tutorialSteps: Record<Language, TutorialStep[]> = {
  en: [
    { id: "download", icon: Download, title: "Download the correct extension", time: "1 minute", body: "Start with the current v1.0.8 ZIP. Strict mode covers each candidate image while it is checked, then resolves to a soft patch over the matching object alone, or a clean reveal. Only located objects are covered, so a check that could not complete leaves the image visible and is reported in the toolbar popup instead. This release defers off-screen work and coalesces repeated checks. The package includes manifest.json and all four icon files.", actions: [{ label: "Download Chrome extension v1.0.8", href: "/storage/content-firewall-chrome-extension_db504c16.zip", download: true }], checklist: ["Save the ZIP, then extract it completely.", "Open the extracted folder and confirm that manifest.json and the icons folder are beside each other."] },
  ...
  ar: [
    { id: "download", icon: Download, title: "نزّل الإضافة الصحيحة", time: "دقيقة واحدة", body: "ابدأ بملف ZIP الحالي الإصدار 1.0.8. وضع Strict يغطي كل صورة مرشحة أثناء الفحص، ثم يتحول إلى تمويه ناعم فوق الكائن المطابق وحده أو كشف الصورة. يُغطّى الكائن المكتشف فقط، فإذا تعذّر إكمال الفحص تبقى الصورة ظاهرة ويُعرض ذلك في نافذة الإضافة. هذا الإصدار يؤجل العمل خارج الشاشة ويدمج الفحوص المتكررة. يحتوي الملف manifest.json وملفات الأيقونة الأربعة.", actions: [{ label: "تنزيل إضافة Chrome الإصدار 1.0.8", href: "/storage/content-firewall-chrome-extension_db504c16.zip", download: true }], checklist: ["احفظ ملف ZIP ثم فك ضغطه بالكامل.", "افتح المجلد الناتج وتأكد أن manifest.json ومجلد icons موجودان بجانب بعضهما."] },
...
export const tutorialInstallGif = "/storage/content-firewall-load-unpacked_a4ea7cf4.gif";
  ...
  return (
    <div className="tutorial-shell" dir={isArabic ? "rtl" : "ltr"}>
      ...
      <main className="tutorial-content">
        ...
        <section className="tutorial-finish"><Eye size={20} /><div><h2>{isArabic ? "ملاحظة مهمة" : "One important note"}</h2><p>{isArabic ? "عند تغيير السياسة، أو عندما تُظهر نافذة الإضافة فحوصاً لم تكتمل بسبب انتهاء الوصول، انسخ السياسة من التطبيق مرة أخرى واستوردها من جديد في الإضافة." : "When you change a policy—or the toolbar popup reports checks that could not complete because access expired—copy the policy again in the web app and re-import it into the extension."}</p></div></section>
      </main>
    </div>
  );
}
